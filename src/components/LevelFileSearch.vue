...
// 关卡下载基础URL
const LEVELS_BASE_URL = 'https://levels.smwp.marioforever.net/Mario Worker 杯关卡/'
...
function isWarmupRound(year: number, roundKey: string): boolean {
  if (roundKey !== 'P1') return false;
  if (!yamlData?.season?.[year]?.rounds?.P1) return false;
  return yamlData.season[year].rounds.P1.is_warmup === true;
}
