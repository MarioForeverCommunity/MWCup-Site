...
<template>
  <div class="content-panel animate-fadeInUp">
    ...
    <div class="filters">
      ...
      <p class="scoring-note">注：评分未结束的轮次的关卡不参与统计；榜单不含 2012 年关卡；2013 半决赛第一轮原始分，娱乐性子项按"娱乐性/60×80"计入；2016 四分之一决赛第二轮原始分，欣赏性子项按"欣赏性×10/15"计入；2025 版大众评选方案的原始分按评委评分去除附加分×75% + 大众评分去除附加分×25%计算；2026 版大众评选方案的原始分按大众评分去除附加分计算。</p>
    </div>
    <!-- 加载状态 -->
...
function downloadLevelFileMulti(item: MultiLevelRankingItem): void {
  ...
  const baseUrl = 'https://levels.smwp.marioforever.net/Mario Worker 杯/';
